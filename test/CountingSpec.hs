...
spec = do
  ...
  it "two territories of same player, rectangular board" $
    [" B "]
      `shouldHaveTerritories` [ ([(1, 1)], Just Black),
                                ([(3, 1)], Just Black)
                              ]
  it "5x5 score" $
    board5x5
      `shouldScore` [ (Nothing, 9),
                      (Just Black, 6),
                      (Just White, 1)
                    ]
  it "5x5 territory for black bordering edge" $
    territoryIn board5x5 (1, 2)
      `shouldBe` Just
        ( [ (1, 1),
            (1, 2),
            (2, 1)
